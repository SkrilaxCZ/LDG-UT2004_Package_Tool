...
from ut2004packageutil.package.package import UnExport, UnPackageItem, resolve_item
...
from ut2004packageutil.utils.struct_utils import format_float32
...
# --display-offset gutter: statement lines are prefixed with this format (the
# begin token's byte offset), and every other line in a function/state block is
# left-padded with an equal-width blank so the code column stays aligned. Both
# end with a tab (not a space) so the following code indents to a tab stop.
_OFFSET_GUTTER_FMT = "/* {:04X} */\t"
_OFFSET_GUTTER_BLANK = " " * len("/* {:04X} */".format(0)) + "\t"
...
def _format_float(value: float) -> str:
    ...
    Delegates to :func:`format_float32`, which renders the shortest decimal that
    round-trips through 32-bit IEEE (so ``0.20000000298023224`` prints as
    ``0.2``) while re-encoding to identical bytes.
...
def _format_string(value: str) -> str:
    ...
    ``t``, ``"\\n"`` to ``n``, verified against a live build). Control characters therefore cannot be written as backslash
    escapes:
    ...
    * LF (0x0A) and CR (0x0D) cannot be raw bytes (they end the line, giving
    ...
    seq: List[Tuple[str, Union[str, int]]] = []
    buf: List[str] = []
    for ch in value:
        if ch == "\n" or ch == "\r":
...
class _BodyDecompiler:
    ...
    def __init__(
        self, decompiler: "Decompiler", struct_obj: UnStruct, container: UnPackageItem
    ) -> None:
        ...
        Args:
            ...
            container (UnPackageItem): The export that owns the struct (used to
        ...
        # When on (only without --simplify), each statement line is prefixed
        # with a ``/* %04X */`` comment giving the begin token's byte offset.
        self.display_offset = decompiler.display_offset
    ...
    def _decompile_nests(self, current: UnToken, flush: bool = False) -> None:
        ...
        for i in range(len(self.nests) - 1, -1, -1):
            ...
            self.ind.remove()
            if n.type == "do":
                cond = self._expr(getattr(n.creator, "condition")) if n.creator else ""
            ...
            if n.has_else is not None:
                ...
                end = _Nest("end", "else", getattr(n.has_else, "offset"), n.has_else)
                ...
                for j, piece in enumerate(line.split("\n")):
                    if j == 0:
                        if self.display_offset:
                            # Left-column byte-offset comment for the token that
                            # begins this statement, then the indented code.
                            self.lines.append(
                                _OFFSET_GUTTER_FMT.format(token.icode_start)
                                + f"{self.ind.tabs}{piece}"
                            )
                        else:
                            self._emit(piece)
                    else:
            ...
            if m:
                ...
                while j < n:
                    ...
                    if lj.strip() == "":  # blank line inside the dead region
                        j += 1
                        continue
                    ws_match = re.match(r"^(\s*)", lj)
                    ljws = ws_match.group(1) if ws_match else ""
    ...
    _BRACE_ELIDE_HDR_RE = re.compile(
        r"^(\t*)(?:(?:else )?if\(.*\)|for\(.*\)|while\(.*\)|foreach .+|else)$"
    )
    ...
    def _elide_single_statement_braces(self, lines: List[str]) -> List[str]:
        ...
        flattened first by _merge_else_if (to a fixpoint, so a whole chain
        collapses), and the resulting ``else if(C)`` sheds its braces too when it
        wraps a single simple statement — binding is unambiguous there.
        """
        # Repeat until stable: each pass folds one ``else { if … }`` level, and a
        # merged block is emitted without being re-scanned, so a chain nested N
        # deep needs N passes to flatten into ``else if`` / ``else if`` / ``else``.
        while True:
            merged = self._merge_else_if(lines)
            if merged == lines:
                break
            lines = merged
        out: List[str] = []
    ...
    def _coerce_operand(
        ...
        ``string(...)`` cast is redundant in source — EXCEPT for conversions
        UnrealScript does *not* coerce automatically, which must keep the
        explicit ``string(...)`` or the source fails to recompile:
          * ``ObjectToString`` — ``"x" $ obj`` is a type error; ``string(obj)``
            is required (an Object has no implicit string coercion);
          * a whole array operand (see :meth:`_expr_is_array`).
        ...
        if (
            ...
            and token.cast_type != int(UnCastType.ObjectToString)
        ...
        # ``Mid(x, 0)`` is ``string(x)``; in a coerce-string param the cast is
        # redundant, so render the bare operand (unless it is a whole array).
        if self.simplify and is_coerce:
            mid_inner = self._mid_zero_inner(token)
            if mid_inner is not None and not self._expr_is_array(mid_inner):
                return (
                    self._precedence_operand(mid_inner, parent_group, is_left)
                    if precedence
                    else self._expr(mid_inner)
                )
        return (
    ...
    def _precedence_operand(
        ...
        *right* operand is safe when the group reassociates (see
        ...
        wrap = False
        # A constant-folded operand renders as a bare literal (``(-1) - 1`` ->
        # ``-2``), so the operator that produced it is gone and parenthesising it
        # would only add noise — match how a plain literal operand renders.
        if self.simplify and self._const_value(token) is not None:
            return text
    ...
    def _effective_operand_token(self, token: Optional[UnToken]) -> Optional[UnToken]:
    ...
    def _is_binary_operator_token(self, token: Optional[UnToken]) -> bool:
    ...
    def _literal_value(self, token: Optional[UnToken]):
    ...
    def _const_value(self, token: Optional[UnToken]):
        ...
        """
        # A Skip wraps the short-circuited RHS of ``&&``/``||``; it is
        # transparent to the operand's value, so see through it when folding.
        while isinstance(token, UnTokenSkip):
            token = token.expression
        ...
        if self.simplify and isinstance(
            ...
            if (
                ...
                and (
                    info.name in self._FOLD_BIN_OPS or info.name in self._FOLD_BOOL_OPS
                )
            ):
                ops = self._operands(token.params)
                if len(ops) == 2:
                    ...
                    if a is not None and b is not None:
                        if info.name in self._FOLD_BOOL_OPS:
                            return self._fold_bool_binary(info.name, a, b)
    ...
    # Logical operators folded to a bool under --simplify when BOTH operands are
    # constant. Only folding constant-vs-constant keeps short-circuit semantics
    # exact (a constant operand has no side effect), so e.g. ``false || false``
    # -> ``false`` and ``int(false || false)`` -> ``0``.
    _FOLD_BOOL_OPS = frozenset({"&&", "||", "^^"})

    def _fold_bool_binary(self, name: str, a, b):
        """Fold ``a <op> b`` for two constant logical operands, or ``None``.

        Both operands are coerced to bool. ``&&`` is logical-and, ``||`` is
        logical-or, ``^^`` is logical-xor.
        """
        av, bv = bool(a[1]), bool(b[1])
        if name == "&&":
            return ("bool", av and bv)
        if name == "||":
            return ("bool", av or bv)
        if name == "^^":
            return ("bool", av != bv)
        return None
    ...
    def _h_final_function(self, t, s):
        ...
        anti-decompilation trap) is emitted as ``super(Owner).Method(...)``
        ...
        * a method of an unrelated class -> either a cross-class ``static.``
        ...
        if idx is None:
            ...
            if (
                ...
                # anti-decompilation traps calling `Engine.Security.LocalPerform`
                # from a `Mutator` subclass). There is no
                ...
                # `ucc` honours the binding, so this form can be shipped
    ...
    def _enclosing_state_item(self):
        """Return the state item that owns the code being decompiled.

        Returns:
            The enclosing ``UnState`` item, or None when the code is not inside
            a state (``UnClass`` derives ``UnState``, so a class is not one).
        """
        cur = self.container
        while cur is not None:
            obj = getattr(cur, "object", None)
            if isinstance(obj, UnState) and not isinstance(obj, UnClass):
                return cur
            cur = cur.group_item
        return None

    def _state_super_prefix(self, item) -> Optional[str]:
        """Return the ``super`` qualifier for a call into an ancestor's STATE.

        A final call can target a function declared inside a *state*
        (``UnrealPlayer.PlayerWaiting.CanRestartPlayer``) rather than directly on
        a class.  From the same-named state of a subclass that is a **state-scope
        super call**, and both of ucc's super forms are state-aware: plain
        ``super`` steps the state's own super-struct,
        and ``super(Class)`` re-scopes to that class's same-named state before
        looking the function up.

        Rendering it as a bare call instead is silently catastrophic: the
        enclosing override has the same name, so the call recurses into itself
        until the VM aborts with *"Infinite script recursion (250/250 calls)"*.

        Args:
            item: The resolved target function item.

        Returns:
            Optional[str]: The qualifier, or None when this is not a call into
                an ancestor's same-named state (leaving the class-based rules to
                decide).
        """
        owner = item.group_item
        if owner is None or self._is_class_item(owner):
            return None
        if not isinstance(getattr(owner, "object", None), UnState):
            return None
        cur_state = self._enclosing_state_item()
        if cur_state is None or cur_state is owner:
            return None
        if (
            cur_state.object_name is None
            or owner.object_name is None
            or cur_state.object_name.name.lower() != owner.object_name.name.lower()
        ):
            # `super(Class).F()` only re-scopes to the state we are *in*, so a
            # call into a differently-named state has no super form.
            return None
        owner_cls = owner.group_item
        cur_cls = self._enclosing_class_item()
        chain = self._ancestor_chain(cur_cls) if cur_cls is not None else []
        idx = self._chain_index(chain, owner_cls)
        if idx is None or idx == 0:
            return None
        return "super." if idx == 1 else f"super({owner_cls.object_name.name})."

    @staticmethod
    def _chain_index(chain: list, owner) -> Optional[int]:
        """Return *owner*'s position in an ancestor *chain*.

        Matches by identity first, then by class name, so an ancestor reached
        through an import still resolves.

        Args:
            chain (list): ``[class, parent, ...]`` as built by
                :meth:`_ancestor_chain`.
            owner: The class item to locate.

        Returns:
            Optional[int]: The index, or None when *owner* is not in the chain.
        """
        if owner is None:
            return None
        for i, node in enumerate(chain):
            if node is owner or (
                node.object_name is not None
    ...
    def _static_call_qualifier(self, item, fn) -> str:
        ...
        cur: Optional["UnPackageItem"] = self.container
        while cur is not None and not isinstance(getattr(cur, "object", None), UnClass):
            cur = cur.group_item
    ...
    def _native_call(self, t, s):
        ...
        # --simplify string-builtin reductions (Mid(x,0)->string(x), etc.).
        if self.simplify:
            reduced = self._simplify_str_builtin(t, info)
    ...
    # String-returning script builtins — used to decide when a ``string(...)``
    # cast around one of them is redundant.
    _STRING_RETURNING_NATIVES = frozenset(
        {"Mid", "Left", "Right", "Caps", "Locs", "Repl", "Chr"}
    )

    @staticmethod
    def _string_literal_value(token: Optional[UnToken]) -> Optional[str]:
        """Return a ``StringConst`` token's value, or None if not one."""
        return token.value if isinstance(token, UnTokenStringConst) else None

    def _yields_string(self, token: Optional[UnToken]) -> bool:
        """Whether ``token`` already evaluates to a string.

        Used to drop a redundant ``string(...)`` wrapper: a string constant, a
        ``*ToString`` cast, or a string-returning builtin needs no cast.
        """
        ...
        if isinstance(token, (UnTokenNativeFunction, UnTokenExtendedNativeFunction)):
            info = self.natives.get(token.native_index)
            if info is not None and info.name in self._STRING_RETURNING_NATIVES:
                return True
        return False

    def _mid_zero_inner(self, token: Optional[UnToken]) -> Optional[UnToken]:
        """Return ``x`` if ``token`` is ``Mid(x, 0)`` (2-arg), else None.

        ``Mid(x, 0)`` returns all of ``x`` coerced to a string, i.e. it is
        exactly ``string(x)``.  A 3-arg ``Mid(x, 0, n)`` is ``Left(x, n)`` and
        ...
        if not isinstance(
            ...
            return None
        info = self.natives.get(token.native_index)
        if info is None or info.name != "Mid":
            return None
        ops = self._operands(token.params)
        if len(ops) != 2:
            return None
        cv = self._const_value(ops[1])
        if cv is not None and cv[0] in ("int", "bool") and int(cv[1]) == 0:
            return ops[0]
        return None

    def _simplify_str_builtin(self, t: UnToken, info) -> Optional[str]:
        """Reduce a string-builtin call under ``--simplify``, or return None.

        * ``Mid(x, 0)`` -> ``string(x)`` (or bare ``x`` when ``x`` already
          yields a string, so the cast is redundant);
        * ``Left(s, N)`` / ``Right(s, N)`` -> ``s`` when ``s`` is a string
          constant and ``N`` equals its length (the whole string, no cast).
        """
        if not isinstance(t, (UnTokenNativeFunction, UnTokenExtendedNativeFunction)):
            return None
        inner = self._mid_zero_inner(t)
        if inner is not None:
            if self._yields_string(inner):
                return self._expr(inner)
            return f"string({self._expr(inner)})"
        if info.name in ("Left", "Right"):
            ops = self._operands(t.params)
            if len(ops) == 2:
                sval = self._string_literal_value(ops[0])
                cv = self._const_value(ops[1])
                if (
                    sval is not None
                    and cv is not None
                    and cv[0] in ("int", "bool")
                    and int(cv[1]) == len(sval)
                ):
                    return self._expr(ops[0])
        return None
            ...
            if 0 < idx < len(self.tokens):
                ...
                if self.positions[idx] == t.offset and isinstance(
                    ...
                    if isinstance(
                        ...
                        prev_token.offset != self.positions[idx]
                        # An `else` is announced by the then-branch jumping
                        # FORWARD over it. A backward jump is the opposite: a
                        # loop back-edge (`continue`, or a `while` re-test) that
                        # merely happens to sit just before this jump's target,
                        # and reading it as an else invents a block that isn't
                        # there. Seen in the wild: several nested tests all
                        # skipping to one address just after a loop's
                        # back-edge, where the invented `else`s swallowed
                        # the `if(!bOK)` guarding a validation report — so
                        # it fired for every input, not just the bad ones.
                        and prev_token.offset > self.positions[idx - 1]
                    ):
                        # if / else
...
class Decompiler:
    ...
    def __init__(
        ...
        Args:
            ...
            reconstruct_loops (bool): When True (and simplify is on),
                ...
                still apply.
            display_offset (bool): When True, prefix each statement line with a
                ``/* %04X */`` comment holding the byte offset of the token that
                begins that statement.  Incompatible with ``simplify`` (which
                reorders statements), so the two must not both be set.

        Raises:
            ValueError: If both ``simplify`` and ``display_offset`` are set.
        """
        if simplify and display_offset:
            raise ValueError(
                "display_offset cannot be combined with simplify "
                "(simplify reorders statements)"
            )
        self.pkg = package
        self.ind = _Indent()
    ...
    @staticmethod
    def _iter_field_items(struct_obj: UnStruct) -> List["UnPackageItem"]:
        """Return the child field items of a struct in stored order.

        Walks the ``Children``/``Next`` linked list Unreal stores a struct's
        fields in.  The order is the reverse of the declaration order in the
        original source.
        ...
        Returns:
            List[UnPackageItem]: The child field exports in stored order.
        """
        items: List["UnPackageItem"] = []
    ...
    def _fields_of_type(self, struct_obj: UnStruct, predicate) -> List["UnPackageItem"]:
        ...
        Returns:
            List[UnPackageItem]: The matching field exports.
        """
    ...
    def _qualify_type(self, outer: UnPackageItem, type_name: str) -> str:
    ...
    def _format_consts(self, struct_obj: UnStruct) -> List[str]:
        ...
        for it in reversed(consts):
            lines.append(self._format_const_decl(it))
        return lines

    def _format_const_decl(self, item: UnPackageItem) -> str:
        """Render a single ``const`` declaration.

        Args:
            item (UnPackageItem): The const export to render.

        Returns:
            str: The ``const NAME = VALUE;`` source line.
        """
        return f"{self.ind.tabs}const {item.object_name.name} = {item.object.value.strip()};"
    ...
    def _format_enums(self, struct_obj: UnStruct) -> List[str]:
        ...
        Returns:
            List[str]: The source lines of all enum definitions, each preceded
                by one blank separator line (as for functions/states), so the
                caller can concatenate blocks without adding its own.
        """
        lines: List[str] = []
        enums = self._fields_of_type(struct_obj, lambda o: isinstance(o, UnEnum))
        for it in reversed(enums):
            lines.append("")
            lines.extend(self._format_enum_def(it))
        return lines

    def _format_enum_def(self, item: UnPackageItem) -> List[str]:
        """Render a single ``enum`` definition.

        Args:
            item (UnPackageItem): The enum export to render.

        Returns:
            List[str]: The enum definition's source lines.
        """
        enum = item.object
        lines = [f"{self.ind.tabs}enum {item.object_name.name}", f"{self.ind.tabs}{{"]
        self.ind.add()
    ...
    def _format_structs(self, struct_obj: UnStruct) -> List[str]:
        ...
        Returns:
            List[str]: The source lines of all nested struct definitions, each
                preceded by one blank separator line (as for functions/states),
                so the caller can concatenate blocks without adding its own.
        """
        lines: List[str] = []
    ...
    def _format_struct_def(self, item: UnPackageItem) -> List[str]:
    ...
    def _format_member_vars(
        ...
        # A class's own variables may be private; a struct's members may not
        # (see :meth:`_format_member_var_decl`).
        in_struct = not isinstance(struct_obj, UnClass)
        ...
        for it in props:
            ...
            if isinstance(prop, UnDelegateProperty):
                ...
                if pname.startswith("__") and pname.endswith("__Delegate"):
                    continue
            lines.append(
                self._format_member_var_decl(prop, owner_name, keyword, in_struct)
            )
        return lines

    @staticmethod
    def _member_access_modifier(prop: UnProperty) -> str:
        """Return a member variable's access specifier, trailing-spaced, or "".

        Unlike a function's, a variable's access control is NOT in its property
        flags — it lives in its **export's object flags**. ``ucc`` compiles
        ``private`` by clearing ``RF_Public`` and setting ``RF_Final``, and
        ``protected`` by setting ``RF_Protected`` (see the variable-specifier
        parsing in the script compiler). ``public`` is the default and is not
        emitted.

        ``private`` requires ``RF_Final`` as well as the absent ``RF_Public``, so
        an export that is merely *hidden* — the obfuscator clears ``RF_Public``
        without setting ``RF_Final`` — is not mistaken for a source-level
        ``private``.

        Args:
            prop (UnProperty): The property being declared.

        Returns:
            str: ``"private "``, ``"protected "``, or ``""``.
        """
        export = getattr(prop, "export", None)
        if export is None:
            return ""
        flags = int(export.flags)
        if flags & int(UnObjectFlags.Private) and not flags & int(UnObjectFlags.Public):
            return "private "
        if flags & int(UnObjectFlags.Protected):
            return "protected "
        return ""

    def _format_member_var_decl(
        self,
        prop: UnProperty,
        owner_name: str,
        keyword: str = "var",
        ...
        """Render one member-variable declaration line.

        Args:
            prop (UnProperty): The property to declare.
            owner_name (str): The name of the struct/class that owns it, used to
                collapse a category equal to the owner to a bare ``var()``.
            keyword (str): The declaration keyword (``"var"`` by default).
            in_struct (bool): True when the declaration is a struct member, whose
                ``private`` is dropped under ``--simplify``. Defaults to False.

        Returns:
            str: The declaration source line, including its trailing ``;``.
        """
        decl = keyword
        cat = prop.category_name_entry
        if cat is not None and cat.name not in ("None", ""):
            decl += "()" if cat.name == owner_name else f"({cat.name})"
        access = self._member_access_modifier(prop)
        if in_struct and self.simplify and access == "private ":
            # `private` on a STRUCT MEMBER does not survive recompilation. ucc
            # parses it happily (it just clears RF_Public on the field), but the
            # member is then unexportable, so the build dies as soon as another
            # package references it — "Referencers of IntProperty
            # Pkg.Class.Struct.Member:" and a non-zero exit, with no line number
            # to go on. Verified on a live OldUnreal build. A class variable is
            # different (its `private` is honoured and enforced), and `protected`
            # is safe either way since it keeps RF_Public.
            access = ""
        return (
            f"{self.ind.tabs}{decl} {access}"
            f"{self._format_property(prop, is_parm=False)};"
        )
    ...
    def _pad_offset_block(self, lines: List[str]) -> List[str]:
        """Left-pad a function/state block so it aligns under the offset gutter.

        Under ``--display-offset`` a statement line already carries a
        ``/* %04X */`` gutter; every other line (declaration header, braces,
        locals, labels, ``else``) gets an equal-width blank so the code column
        lines up. Blank lines, guttered statement lines (``/* ``) and
        already-blank-padded lines are left untouched — the last guard makes
        this idempotent, so a function block embedded in a state is not padded
        twice.

        Args:
            lines (List[str]): The rendered block lines.

        Returns:
            List[str]: The padded lines (unchanged if display_offset is off).
        """
        if not self.display_offset:
            return lines
        padded: List[str] = []
        for ln in lines:
            if ln == "" or ln.startswith("/* ") or ln.startswith(_OFFSET_GUTTER_BLANK):
                padded.append(ln)
            else:
                padded.append(_OFFSET_GUTTER_BLANK + ln)
        return padded

    def _format_function(self, item: UnPackageItem) -> List[str]:
        ...
        if is_native:
            return self._pad_offset_block([f"{self.ind.tabs}{header};"])

        self.ind.add()
        local_lines: List[str] = []
        for local in self._function_locals(fn):
            # A local's access specifier is noise in the same way its type
            # modifiers are (it says nothing the body does not), so --simplify
            # drops both.
            access = "" if self.simplify else self._member_access_modifier(local)
            local_lines.append(
                f"{self.ind.tabs}local {access}"
        ...
        lines.append(f"{self.ind.tabs}}}")
        return self._pad_offset_block(lines)
    ...
    def _format_state(self, item: UnPackageItem) -> List[str]:
        ...
        self.ind.remove()
        lines.append(f"{self.ind.tabs}}}")
        # Embedded function lines are already padded by _format_function; the
        # blank-pad guard in _pad_offset_block keeps this idempotent for them.
        return self._pad_offset_block(lines)
    ...
    def _class_is_actor(self, class_obj: UnClass) -> bool:
        ...
        """
        obj: Optional[UnClass] = class_obj
        seen: set = set()
    ...
    def _enum_prop_names(self, class_obj: UnClass) -> dict:
        ...
        """
        out: dict = {}
        obj: Optional[UnStruct] = class_obj
        seen: set = set()
        while isinstance(obj, UnStruct):
            for it in self._iter_field_items(obj):
                p = it.object
                if isinstance(p, UnByteProperty) and p.enum_item is not None:
    ...
    def _static_array_prop_names(self, class_obj: UnClass) -> set:
        ...
        while isinstance(obj, UnStruct):
            for it in self._iter_field_items(obj):
                p = it.object
                if isinstance(p, UnProperty) and p.array_dim and p.array_dim > 1:
    ...
    def _dp_lines(
        ...
        is_static = static_arrays is not None and name in static_arrays
    ...
    def _begin_class(self, item: UnExport) -> None:
        """Reset the per-class rendering context for a class export.

        Every renderer that resolves a type name, a ``dependsOn`` target or a
        default-property value reads this context, so it must be armed before
        rendering a class or any single member of one.

        Args:
            item (UnExport): The class export about to be rendered.
        """
        self.ind.level = 0
        class_obj = item.object
    ...
    def decompile_class(self, item: UnExport) -> str:
        ...
        # Enums, structs, functions and states carry their own leading blank
        # separator per item, so they are concatenated as-is; consts and member
        # vars are single unseparated groups and get one separator here.
        lines.extend(enums)
        ...
        # Uniform separation: exactly one blank line between blocks anywhere in
        # the file. Dropping a class's only consts (above) leaves that block's
        # separator behind, which would otherwise double up with the next
        # block's.
        return re.sub(r"\n{3,}", "\n\n", source)
