...
def _iter_children(struct_obj: UnStruct) -> List["UnPackageItem"]:
    ...
    Returns:
        List[UnPackageItem]: The child field export items.
    """
    out: List["UnPackageItem"] = []
...
def _enclosing_class(item: "UnExport") -> Optional["UnClass"]:
    ...
    Returns:
        Optional[UnClass]: The owning class object, or None if not found.
    """
    cur = item.group_item
...
def _hierarchy_field_names(
    ...
    names: Set[str] = set()
    seen: Set[int] = set()
    cur: Optional[UnStruct] = class_obj
...
def apply_name_map(pkg: UnPackage, mapping: Dict[str, str]) -> DeobfuscationReport:
    ...
    overrides: List[Tuple["UnPackageItem", str]] = []  # (property_export, final_name)
    ...
    for exp in pkg.exports:
        ...
        params: List["UnPackageItem"] = []
        locals_: List["UnPackageItem"] = []
        for child in _iter_children(obj):
            co = child.object
