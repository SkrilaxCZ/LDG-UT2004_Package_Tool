...
class PackageLoader:
    ...
    def load_with_dependencies(
        ...
        *,
        ...
        try:
            ...
            self._wire_dependencies(pkg, pkg_name, parse_objects=parse_objects)
            return pkg
        finally:
            self._loading_in_progress.discard(pkg_name)

    def load_packages_with_dependencies(
        self,
        file_paths: List[str],
        *,
        parse_objects: bool = True,
    ) -> List[UnPackage]:
        """Load several primary packages together, cross-wiring them.

        Every listed package is read **fully** (objects parsed when
        ``parse_objects`` is True) and cached *before* any dependency
        resolution runs.  That ordering matters: plain
        :meth:`load_with_dependencies` loads a dependency header-only, so
        loading ``A`` as a dependency of ``B`` would leave the shared cache
        holding an object-less ``A``.  Here each listed package sees its
        listed siblings as fully-parsed instances, which is what the
        obfuscator needs to rewrite a whole package family in one pass.

        Note that a package is matched to its siblings' import entries by
        its **file basename** — pass the files named exactly as the imports
        spell them (stage a copy if the on-disk build carries a suffix such
        as ``-dev``), otherwise a sibling's import resolves through the
        search paths to a different build of that package.

        Args:
            file_paths (List[str]): Paths of the primary packages to load.
            parse_objects (bool): Whether to parse objects in the primary
                packages. Defaults to True.

        Returns:
            List[UnPackage]: The loaded packages, in the order requested.

        Raises:
            FileNotFoundError: When a dependency contributing code imports
                cannot be located on disk.
        """
        primaries: List[Tuple[str, UnPackage]] = []
        for file_path in file_paths:
            pkg_name = os.path.splitext(os.path.basename(file_path))[0]
            cached = self.loaded_packages.get(pkg_name)
            if cached is not None:
                primaries.append((pkg_name, cached))
                continue
            primaries.append(
                (pkg_name, self.load_package(file_path, parse_objects=parse_objects))
            )

        for pkg_name, pkg in primaries:
            self._loading_in_progress.add(pkg_name)
        try:
            for pkg_name, pkg in primaries:
                self._wire_dependencies(pkg, pkg_name, parse_objects=parse_objects)
        finally:
            for pkg_name, _ in primaries:
                self._loading_in_progress.discard(pkg_name)

        return [pkg for _, pkg in primaries]
    ...
    def _wire_dependencies(
        ...
        *,
        ...
        """Discover, load and attach ``pkg``'s dependency packages.

        Args:
            pkg (UnPackage): The already-read package to wire up.
            pkg_name (str): The package's own (file-derived) name.
            parse_objects (bool): Whether ``pkg`` was read with its objects
                parsed (controls the re-resolve at the end).
