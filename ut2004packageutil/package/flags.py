...
class UnObjectFlags(IntFlag):
    ...
    # RF_Final ("not visible outside of class") — what `ucc` sets, together with
    # clearing Public, for a variable declared `private`.
    Private = 0x00000080
    Automated = 0x00000100
    NeedLoad = 0x00000200
    HighlightedName = 0x00000400
    InSingularFunc = 0x00000800
    # RF_Protected: a property declared `protected`.  The engine overloads
    # 0x800 (RF_InSingularFunc / RF_RemappedName / RF_Protected), so this is an
    # alias — which reading applies depends on the object's kind.
...
def _string_to_flags(
    s: str, flag_map: Dict[str, _FlagT], cls: Type[_FlagT], label: str
) -> _FlagT:
    ...
    Returns:
        The combined flag value parsed from the string (an instance of *cls*).
...
_FUNCTION_FLAG_MAP = {m.name: m for m in UnFunctionFlags if m.name is not None}

_STATE_FLAG_MAP = {m.name: m for m in UnStateFlags if m.name is not None}

_PROPERTY_FLAG_MAP = {m.name: m for m in UnPropertyFlags if m.name is not None}

_STRUCT_FLAG_MAP = {m.name: m for m in UnStructFlags if m.name is not None}

_CLASS_FLAG_MAP = {m.name: m for m in UnClassFlags if m.name is not None}
