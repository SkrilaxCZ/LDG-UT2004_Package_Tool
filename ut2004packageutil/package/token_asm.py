...
def _emit_token(
    ...
    Args:
        ...
        display_offset (bool): When True, prefix each line with a
            ``/* %04X */`` comment (column 0, before indentation) holding the
            token's byte offset (from the ``_icode`` field).
    """
    parts: List[str] = []
    if role:
    ...
    prefix = ""
    if display_offset:
        icode = token.get("_icode", 0)
        prefix = f"/* {int(icode):04X} */ "
    lines.append(prefix + _INDENT * indent + " ".join(parts))
    for child_role, child_is_list, child in children:
        _emit_token(child, indent + 1, child_role, child_is_list, lines, display_offset)


def tokens_to_asm(
    token_dicts: List[Dict[str, Any]], display_offset: bool = False
) -> str:
    ...
    Args:
        ...
        display_offset (bool): When True, prefix every line with a
            ``/* %04X */`` byte-offset comment (requires the dicts to carry
            ``_icode`` fields, i.e. produced with the parser's ``_emit_icode``
            flag set).
    ...
    for token in token_dicts:
        _emit_token(token, 0, "", False, lines, display_offset)
    return "\n".join(lines) + "\n"
...
def asm_to_tokens(text: str) -> List[Dict[str, Any]]:
    ...
    for raw_line in text.splitlines():
        # Drop a leading ``/* %04X */`` display-offset comment (emitted at
        # column 0 by tokens_to_asm(display_offset=True)); the token's own
        # indentation follows it, so strip only the comment and its trailing
        # space, preserving the indentation used for nesting.
        offset_match = re.match(r"/\*[^*]*\*/ ?", raw_line)
        if offset_match:
            raw_line = raw_line[offset_match.end() :]
