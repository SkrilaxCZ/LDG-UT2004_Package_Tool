...
class UnPackageIO:
    ...
    def _write_imports(pkg: "UnPackage", writer: BinaryIO) -> None:
        ...
        for imp in pkg.imports:
            assert (
                imp.class_package_name is not None and imp.class_name is not None
            ), "import class/package name pointers must be resolved before writing"
    ...
    def export_xml(
        ...
        Args:
            ...
            display_offset (bool): When True, token disassembly sidecars prefix
                each line with a ``/* %04X */`` byte-offset comment (ignored on
                import).
        """
        ...
        for export in pkg.exports:
            ...
            if export.object is not None:
                # When display-offset is requested, arm the struct's token
                # parser so its disassembly dict carries per-token ``_icode``.
                token_parser = getattr(export.object, "token_parser", None)
                if token_parser is not None:
                    token_parser._emit_icode = display_offset
                ...
                # An export whose class the tool does not model but whose bytes
                # are a pure tagged-property container (e.g. an embedded GUI
                # component) is emitted as structured <ObjectData>, exactly like
                # defaultproperties, instead of an opaque Raw<Class>/*.bin blob.
                # Anything with trailing native data (textures, meshes, …) or
                # that does not round-trip losslessly falls through to raw.
                from ut2004packageutil.package.object import build_component_object

                component = build_component_object(export)
                if component is not None:
                    data_el = ET.SubElement(el, "ObjectData")
                    dict_to_xml(data_el, component.to_dict())
                    continue
    ...
    def import_xml(self, input_dir: str) -> "UnPackage":
        ...
        if version_el is None or version_el.text is None:
        ...
        if lm_el is None or lm_el.text is None:
        ...
        # Exports that were structured as pure property containers on export
        # (their class is unknown to the factory, so create_object left them
        # None) are re-created as UnComponentObject from their <ObjectData>.
        for i, export in enumerate(pkg.exports):
            if export.object is not None:
                continue
            obj_dict = export_object_dicts[i] if i < len(export_object_dicts) else None
            if obj_dict is not None and obj_dict.get("type") == "UnComponentObject":
                from ut2004packageutil.package.object import UnComponentObject

                export.object = UnComponentObject(export)
        for i, export in enumerate(pkg.exports):
            obj = export.object
            if obj is None:
                continue
