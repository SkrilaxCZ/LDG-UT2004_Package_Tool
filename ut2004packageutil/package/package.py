...
if TYPE_CHECKING:
    # Referenced only in quoted annotations; object.py imports this module (not
    # vice versa), so a TYPE_CHECKING-guarded import avoids the runtime cycle.
    from ut2004packageutil.package.object import UnObject
...
class UnPackageItem(ABC):
    ...
    def object(self) -> Any:
        ...
        package and returns its object. Typed as :data:`Any` (rather than
        ``Optional[UnObject]``) because the concrete type varies per item and
        callers access it structurally; ``is not None`` checks still work.

        Returns:
            Any: The parsed object, or None when unavailable.
        """
        ...
...
class UnImport(UnPackageItem):
    ...
    def object(self) -> Any:
        ...
        Returns:
            Any: The resolved object from the dependent package,
...
class UnExport(UnPackageItem):
    ...
    def object(self) -> Any:
        ...
        Typed as :data:`Any` (rather than ``Optional[UnObject]``) because the
        concrete object type varies per export and callers access it
        structurally; ``is not None`` checks still work.

        Returns:
            Any: The parsed object, or None when not yet created.
        """
        return self._object
...
class UnPackage:
    ...
    def deduplicate_names(self) -> None:
        ...
        for imp in self.imports:
            if imp.object_name is not None:
                imp.object_name = self._resolve_existing_name(imp.object_name.name)
        ...
        for export in self.exports:
            if export.object_name is not None:
                export.object_name = self._resolve_existing_name(
    ...
    def _resolve_existing_name(self, name: str) -> "UnName":
        """Re-resolve a name that must already exist in the table.

        Used when re-pointing an item's ``object_name`` after the table was
        rebuilt: the entry is known to exist, so a missing match is a bug.

        Args:
            name (str): The name string to look up.

        Returns:
            UnName: The matching entry.

        Raises:
            AssertionError: If no entry matches (indicates table corruption).
        """
        entry = self.find_name(name)
        assert entry is not None, f"name {name!r} missing from the rebuilt table"
        return entry
