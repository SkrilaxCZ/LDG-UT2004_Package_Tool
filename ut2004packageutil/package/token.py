...
if TYPE_CHECKING:
    # Referenced only in quoted annotations; guarded to avoid a runtime cycle.
...
class UnTokenFinalFunction(UnToken):
    ...
    def from_dict(self, data: Dict[str, Any], parser: "TokenStreamParser") -> None:
        ...
        self.params = [
            ...
            if (tok := parser.token_from_dict(p)) is not None
        ]
...
class TokenStreamParser:
    ...
    def __init__(self, package: "UnPackage") -> None:
        ...
        # When True, :meth:`token_to_dict` annotates every token dict with an
        # ``_icode`` key holding the token's byte offset.  Consumed by the
        # assembler renderer to emit ``/* %04X */`` line comments; left False
        # for normal (round-trippable) export.
        self._emit_icode: bool = False
    ...
    def token_to_dict(self, token: Optional[UnToken]) -> Dict[str, Any]:
        ...
        if refs and token.icode_start in refs:
            ...
            d = new_d
        if self._emit_icode:
            # Prepend the token's byte offset so the assembler renderer can
            # emit it as a ``/* %04X */`` line comment.  Never read back on
            # import (the comment is stripped before parsing).
            d = {"_icode": token.icode_start, **d}
        return d
    ...
    def parse_expr(self, reader: BinaryIO, icode: int) -> Tuple[UnToken, int]:
        ...
        if token_byte >= UnScriptTokenType.FirstNative:
            native = UnTokenNativeFunction()
            native.icode_start = icode_start
            native.native_index = token_byte
            native.params = []
            while True:
                param, icode = self.parse_expr(reader, icode)
                native.params.append(param)
                if isinstance(param, UnTokenEndFunctionParms):
                    break
            icode = self.handle_optional_debug_info(reader, icode, native)
            return native, icode

        # Extended native with id 256-16383 (opcode 0x60-0x6F + second byte)
        if token_byte >= UnScriptTokenType.ExtendedNative:
            ext = UnTokenExtendedNativeFunction()
            ext.icode_start = icode_start
            second_byte = read_byte(reader)
            icode += SIZE_BYTE
            ext.native_index = (token_byte - 0x60) * 256 + second_byte
            ext.params = []
            while True:
                param, icode = self.parse_expr(reader, icode)
                ext.params.append(param)
                if isinstance(param, UnTokenEndFunctionParms):
                    break
            icode = self.handle_optional_debug_info(reader, icode, ext)
            return ext, icode
    ...
    def serialize_expr(
        self, writer: BinaryIO, token: Optional["UnToken"], icode: int
    ) -> int:
        ...
        Args:
            ...
            token (Optional[UnToken]): The token to serialize. Must not be
                None (accepted as Optional only because the token fields that
                feed it are None-initialised then populated at load time).
            icode (int): Current instruction-code offset.
        ...
        """
        if token is None:
            raise ValueError("serialize_expr called with a None token")
